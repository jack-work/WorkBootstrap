...
function Install-ScoopPackages {
    ...
    foreach ($package in $packages) {
        try {
            ...
            $bucket = if ($package.bucket -and $package.bucket -ne 'main') { $package.bucket } else { $null }
...
function Install-LanguageServers {
    ...
    foreach ($npmPackage in $npmPackages) {
        try {
            ...
            $installGlobally = if ($npmPackage.global -ne $null) { $npmPackage.global } else { $true }
...
function Main {
    ...
    if ($All -or $CloneRepos) {
        Clone-GitRepositories -CloneDir $CloneDirectory
    }
